...
public final class FakeLikes extends MargyPlugin {
    ...
    @Override public void onStart(Context c){
        ...
        running=true; margyt().log("MULTITOOL_READY author=Zftoz version=0.0.7 target="+likes()+" animation="+animation()+" speed="+speed()+"%");
    }
    ...
    private long duration(long b){return Math.max(90,b*100L/speed());}

    private void attach(final Activity a,String why){
        activity=a;margyt().log("MULTITOOL_ACTIVITY "+why+"="+a.getClass().getName());
        a.getWindow().getDecorView().postDelayed(new Runnable(){public void run(){if(running&&activity==a)scan(a);}},300);
    ...
    private int findLikes(View v){
        int n=0;
        if(v instanceof TextView){TextView t=(TextView)v;if(isLikeResource(t)){n++;watch(t);animateToTarget(t);}}
    ...
    private int findLabelLikes(View root){
        if(!(root instanceof ViewGroup))return 0;ArrayList<TextView> labels=new ArrayList<TextView>();collectText(root,labels);int n=0;
        for(TextView label:labels){if(!isLikesLabel(label))continue;TextView target=nearestNumber(root,label);if(target!=null&&!isLikeResource(target)){watch(target);animateToTarget(target);n++;}}
        return n;
    }
    private TextView nearestNumber(View root,TextView label){
        ArrayList<TextView> all=new ArrayList<TextView>();collectText(root,all);Rect lr=new Rect();label.getGlobalVisibleRect(lr);float lx=lr.centerX(),ly=lr.centerY();TextView best=null;double bestD=Double.MAX_VALUE;
        for(TextView t:all){if(t==label||!isNumberText(t)||isLikesLabel(t))continue;Rect tr=new Rect();t.getGlobalVisibleRect(tr);float dx=tr.centerX()-lx,dy=tr.centerY()-ly;double d=Math.sqrt(dx*dx+dy*dy);if(d<bestD){bestD=d;best=t;}}
        return bestD<=320?best:null;
    }
    ...
    private void watch(final TextView t){
        ...
        t.addTextChangedListener(new TextWatcher(){private boolean self;public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){if(self||!t.isShown()||!running)return;final String target=formatCount(likes());if(target.equals(String.valueOf(s)))return;t.post(new Runnable(){public void run(){if(running)animateToTarget(t);}});}public void afterTextChanged(android.text.Editable e){}});
    }
    private void animateToTarget(final TextView t){
        if(t==null||!t.isShown())return;final String target=formatCount(likes());String current=String.valueOf(t.getText());if(target.equals(current))return;String mode=animation();
        if("stock".equals(mode)){t.setText(target);return;}long start=parseCount(current),end=likes();if(start<0){t.setText(target);return;}
        ValueAnimator old=textAnim.get(t);if(old!=null)old.cancel();ValueAnimator v=ValueAnimator.ofFloat(start,end);v.setDuration(duration("bounce".equals(mode)?720:560));
        if("spring".equals(mode))v.setInterpolator(new OvershootInterpolator(2.2f));else if("bounce".equals(mode))v.setInterpolator(new BounceInterpolator());else if("ease".equals(mode))v.setInterpolator(new AccelerateDecelerateInterpolator());else v.setInterpolator(new DecelerateInterpolator(1.6f));
        v.addUpdateListener(x->{if(!running||!t.isShown())return;t.setText(formatCount(Math.round((Float)x.getAnimatedValue())));});v.start();textAnim.put(t,v);margyt().log("MULTITOOL_ANIM mode="+mode+" from="+current+" to="+target);if("pulse".equals(mode))pulse(t);
    }
    private long parseCount(String s){try{String x=s.trim().replace(",","").replace(" ","").toUpperCase(Locale.US);double m=1;if(x.endsWith("K")){m=1000;x=x.substring(0,x.length()-1);}else if(x.endsWith("M")){m=1000000;x=x.substring(0,x.length()-1);}else if(x.endsWith("B")){m=1000000000;x=x.substring(0,x.length()-1);}return Math.round(Double.parseDouble(x)*m);}catch(Throwable e){return -1;}}
    ...
    private void pulse(View v){v.animate().cancel();v.setScaleX(1f);v.setScaleY(1f);v.animate().scaleX(1.08f).scaleY(1.08f).setDuration(duration(120)).withEndAction(()->v.animate().scaleX(1f).scaleY(1f).setDuration(duration(260)).start()).start();}
    ...
    private void animateIcon(View v){long now=System.currentTimeMillis(),last=iconAnim.containsKey(v)?iconAnim.get(v):0L;if(now-last<1400)return;iconAnim.put(v,now);String mode=animation();v.animate().cancel();v.setScaleX(1f);v.setScaleY(1f);if("spring".equals(mode))v.animate().scaleX(1.18f).scaleY(1.18f).setInterpolator(new OvershootInterpolator(2.5f)).setDuration(duration(260)).withEndAction(()->v.animate().scaleX(1f).scaleY(1f).setDuration(duration(320)).start()).start();else if("bounce".equals(mode))v.animate().scaleX(1.14f).scaleY(1.14f).setInterpolator(new BounceInterpolator()).setDuration(duration(500)).withEndAction(()->v.animate().scaleX(1f).scaleY(1f).setDuration(duration(220)).start()).start();else v.animate().scaleX(1.10f).scaleY(1.10f).setDuration(duration(160)).withEndAction(()->v.animate().scaleX(1f).scaleY(1f).setDuration(duration(280)).start()).start();}
    ...
    private View findCommentStickerControl(View root,boolean comment){
        if(!comment)return null;EditText composer=findCommentComposer(root);
        if(composer!=null){ViewGroup p=composer.getParent() instanceof ViewGroup?(ViewGroup)composer.getParent():null;for(int depth=0;depth<5&&p!=null;depth++,p=p.getParent() instanceof ViewGroup?(ViewGroup)p.getParent():null){View x=findStickerInGroup(p);if(x!=null)return x;}}
        return findStickerInTree(root);
    }
    ...
    private boolean isStickerView(View v){
        if(v==null||!v.isShown())return false;String r="",c="",cls=v.getClass().getName().toLowerCase(Locale.US);if(v.getId()!=View.NO_ID)try{r=v.getResources().getResourceName(v.getId()).toLowerCase(Locale.US);}catch(Throwable e){}if(v.getContentDescription()!=null)c=v.getContentDescription().toString().toLowerCase(Locale.US);
        boolean word=r.contains("sticker")||r.contains("emoji")||r.contains("emoticon")||r.contains("gif")||c.contains("sticker")||c.contains("emoji")||c.contains("emoticon")||c.contains("gif")||c.contains("стикер")||c.contains("емодзи");
        boolean type=cls.contains("sticker")||cls.contains("emoji")||cls.contains("emoticon");return(word||type)&&!(r.contains("count")||r.contains("comment_count"));
    }
    ...
    private void ensureStickerButton(Activity a,View root,View nativeControl){
        if(findTag(root,STICKER_TAG)!=null)return;EditText composer=findCommentComposer(root);View anchor=nativeControl;ViewGroup parent=null;if(anchor!=null)parent=anchor.getParent() instanceof ViewGroup?(ViewGroup)anchor.getParent():null;if(parent==null&&composer!=null)parent=composer.getParent() instanceof ViewGroup?(ViewGroup)composer.getParent():null;if(parent==null)return;
        TextView b=interfaceButton(a,STICKER_TAG,"Стикер");final View fallback=nativeControl;b.setOnClickListener(v->{View target=fallback!=null?fallback:findCommentStickerControl(a.getWindow().getDecorView(),true);if(target!=null){boolean ok=false;try{ok=target.performClick();}catch(Throwable e){}if(!ok)try{ok=target.callOnClick();}catch(Throwable e){}margyt().log("MULTITOOL_STICKER_OPEN found=true opened="+ok+" class="+target.getClass().getName());}else{Toast.makeText(a,"Кнопка стикеров TikTok не найдена",Toast.LENGTH_SHORT).show();margyt().log("MULTITOOL_STICKER_OPEN found=false");}});
        int at=parent.indexOfChild(anchor!=null?anchor:composer);parent.addView(b,Math.min(Math.max(0,at),parent.getChildCount()));margyt().log("MULTITOOL_STICKER_BUTTON added=interface native="+(nativeControl!=null));
    }
    private TextView interfaceButton(Activity a,String tag,String text){TextView b=new TextView(a);b.setTag(tag);b.setText(text);b.setTextColor(Color.WHITE);b.setTextSize(13);b.setGravity(Gravity.CENTER);b.setPadding(18,10,18,10);b.setMinHeight(42);GradientDrawable bg=new GradientDrawable();bg.setColor(Color.rgb(35,35,40));bg.setCornerRadius(16);b.setBackground(bg);if(!"stock".equals(animation())){b.setAlpha(0f);b.animate().alpha(1f).setDuration(duration(260)).start();}return b;}
    ...
    private void dialog(final Activity a){
        ...
        d.setOnShowListener(x->d.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{try{int value=Integer.parseInt(e.getText().toString().trim());String mode=animation();int id=modes.getCheckedRadioButtonId();if(id>=1000&&id<1000+values.length)mode=values[id-1000];margyt().prefs().edit().putInt(LIKES,Math.max(0,value)).putInt(SPEED,sb.getProgress()+50).putString(ANIM,mode).apply();Toast.makeText(a,"Сохранено",Toast.LENGTH_SHORT).show();d.dismiss();}catch(Throwable ex){Toast.makeText(a,"Введите число",Toast.LENGTH_SHORT).show();}}));d.show();
    }
}
